...
function getValidationQuery(target, constraints, req) {
  const columns = []
  const paramColumns = []
  ...
  // const validationQuery = SELECT.from(req.subject).columns(columns).where(keyMatchingCondition)
  const validationQuery = SELECT.from(req.subject).columns(columns)
  const prop = req.query.UPDATE || req.query.UPSERT
  if(prop?.where) {
    validationQuery.where = prop.where
  }
